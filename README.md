...
The tool generates each year's doubles to satisfy that rotation, with hard-avoid for the last 2 seasons and soft-avoid for the 3rd. Every doubled pair plays in week 1–3 *and* in week 12–14, giving the maximum-possible 11-week gap between the two meetings.
...
- **3-year lookback.** Last 2 seasons are hard-avoid (cannot repeat). The 3rd season back is soft-avoid (preferred to skip, but allowed when the constraint set is too tight).
...
1. Visit the [DoubleCheck site](#) (URL coming with Phase 5 deploy).
